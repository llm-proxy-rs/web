...
test.describe("gateway settings", () => {
  test("UF-32 shows Renew API Key button when gateway is configured", async ({
    page,
  }) => {
    await setupApp(page, {
      settings: {
    ...
    await expect(
      page.getByRole("button", { name: "Renew API Key" }),
    ).toBeVisible();
    await expect(
      page.getByText("Your API key is managed automatically"),
    ).toBeVisible();
  });

  test("UF-33 hides manual API key input when gateway is configured", async ({
    page,
  }) => {
    await setupApp(page, {
      settings: {
  ...
  test("UF-34 clicking Renew API Key shows success message", async ({
    page,
  }) => {
  ...
  test("UF-35 server error when renewing shows failure message", async ({
    page,
  }) => {
    await setupApp(page, {
      settings: {
  ...
  test("UF-36 server redirect response triggers navigation", async ({
    page,
  }) => {
    const ctrl = await setupApp(page, {
      settings: {
      ...
      renewGatewayKeyRedirect:
        "https://auth.example.com/oauth2/authorize?client_id=test",
    });
    ...
    await expect(
      page.getByText("API key renewed successfully."),
    ).not.toBeVisible();
    await expect(page.getByText("Failed to renew.")).not.toBeVisible();
  });

  test("UF-37 shows Set badge when gateway configured and has_api_key is true", async ({
    page,
  }) => {
    await setupApp(page, {
      settings: {
  ...
  test("UF-38 manual input still shown when gateway is not configured", async ({
    page,
  }) => {
    await setupApp(page, {
      settings: {
