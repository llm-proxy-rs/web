...
test.describe("pending-session", () => {
  test("PS-01 title appears in sidebar immediately on send", async ({
    page,
  }) => {
    await setupApp(page, {});
    ...
    await expect(
      page.locator("span.truncate").filter({ hasText: "Hello Claude" }),
    ).toBeVisible();
  });

  test("PS-02 clicking New Chat while streaming keeps title row with pulsing indicator", async ({
    page,
  }) => {
    await setupApp(page, {});
  ...
  test("PS-03 done event replaces user-message title with real session title", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("span.truncate").filter({ hasText: "Hello" }),
    ).toBeVisible();

    ctrl.setSessions([
      makeSession({ session_id: "sess-1", title: "Hello session" }),
    ]);
    ctrl.sendSseEvents(sse.text("Hi there!", "sess-1"));

    // Real session title should replace the user-message title
    await expect(
      page.locator("span.truncate").filter({ hasText: "Hello session" }),
    ).toBeVisible();
    await expect(
      page.locator("span.truncate").filter({ hasText: /^Hello$/ }),
    ).not.toBeVisible();
  ...
  test("PS-04 clicking title row while streaming shows in-progress messages", async ({
    page,
  }) => {
    await setupApp(page, {});
  ...
  test("PS-05 POST /chat error: error shown, composer re-enables", async ({
    page,
  }) => {
