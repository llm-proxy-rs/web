...
test.describe("quick settings", () => {
  ...
  test("QS-05 toggling show thinking off hides thinking blocks", async ({ page }) => {
    // Pre-seed showThinking = false
    await page.addInitScript(() => {
      localStorage.setItem(
        "ui_preferences",
        JSON.stringify({ autoExpandTools: false, showThinking: false, autoScrollToBottom: false }),
      );
    });
  ...
  test("QS-06 toggling auto-expand tools on expands tool cards by default", async ({ page }) => {
    // Pre-seed autoExpandTools = true
    await page.addInitScript(() => {
      localStorage.setItem(
        "ui_preferences",
        JSON.stringify({ autoExpandTools: true, showThinking: false, autoScrollToBottom: false }),
      );
    });
    ...
    app.sendSseEvents(
      sse.withTool("tool-1", "Bash", { command: "ls -la" }, "file1.txt\nfile2.txt", "Done!", "sess-2"),
    );
    ...
    await expect(page.locator("pre").filter({ hasText: "ls -la" })).toBeVisible();
  });
});
