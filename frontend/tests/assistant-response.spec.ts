...
test.describe("assistant response - status indicator phases", () => {
  test("shows 'Thinking' status during init/thinking phase", async ({ page }) => {
  ...
  test("shows 'Responding' status when text_delta arrives", async ({ page }) => {
    const ctrl = await setupApp(page, { sessions: [] });
  ...
  test("shows tool name in status when tool_start arrives", async ({ page }) => {
    const ctrl = await setupApp(page, { sessions: [] });
    ...
    ctrl.sendSseEvents([
      { event: "session_start", data: { task_id: "task-1" } },
      { event: "init" },
      { event: "tool_start", data: { id: "t1", name: "Bash", input: { command: "ls" } } },
    ]);
  ...
  test("status returns to Thinking after tool completes and before next text", async ({ page }) => {
    const ctrl = await setupApp(page, { sessions: [] });
    ...
    ctrl.sendSseEvents([
      ...
      { event: "init" },
      { event: "tool_start", data: { id: "t1", name: "Bash", input: { command: "ls" } } },
      { event: "tool_result", data: { tool_use_id: "t1", content: "file.txt", is_error: false } },
...
test.describe("assistant response - status position", () => {
  test("status indicator appears above the composer, not inside message scroll area", async ({ page }) => {
    const ctrl = await setupApp(page, { sessions: [] });
...
test.describe("assistant response - message grouping", () => {
  test("assistant text after tool use does not show a second Claude header", async ({ page }) => {
    const ctrl = await setupApp(page, { sessions: [] });

    await sendMessage(page, "Run ls");
    ctrl.sendSseEvents(
      sse.withTool("t1", "Bash", { command: "ls" }, "file.txt", "Here are the files.", "sess-1"),
    );
    ...
    const claudeHeaders = page.locator("text=Claude").filter({ has: page.locator("xpath=..") });
    // The "Claude" label in the header
    const headerLabels = page.locator('[class*="font-semibold"]').filter({ hasText: "Claude" });
    const count = await headerLabels.count();
    expect(count).toBe(1);
  });

  test("multiple text chunks in one response stay under single Claude header", async ({ page }) => {
    const ctrl = await setupApp(page, { sessions: [] });
